...
三级候选采集项组见 `docs/linux-runtime-info-collection-items.md`。最终采集范围应在后续采集决策文档中从三级项筛选，不直接回到二级分类临时发散。
...
#### RT-20-07 有明确 OS 子系统语义的系统守护进程内存事件队列

- 判定标准：信息描述 systemd 组件等有明确 OS 子系统语义的系统守护进程尚未持久化或尚未处理的内存事件队列和状态缓冲，不包含任意守护进程的应用内部业务队列。

#### RT-20-08 rate limit、抑制与丢失事件状态

- 判定标准：信息描述内核或有明确 OS 子系统语义的系统守护进程因限流、抑制、丢弃而造成的事件缺口和当前计数。
...
#### RT-21-08 有明确 OS 子系统语义的系统守护进程派生缓存

- 判定标准：信息描述 systemd-resolved、nscd 等有明确 OS 子系统语义的系统守护进程为了加速 OS 级解析、寻址、访问或状态展示而维护的临时缓存。
